# Тестовое приложение для кандидатов Vyatka IT
...
### Среда разработки (без сборки нативного кода):
   ```bash
   # iOS
   npx expo start --ios
   ```
...
### Локальная сборка:
   ```bash
   # iOS (debug)
   npm run build:ios:develop
   ```
   ```bash
   # Android (debug)
   npm run build:android:develop
   ```
   ```bash
   # iOS (release)
...
## Узнать больше про фреймворк Expo
